...
found that the current state of disperate data systems across our hotel chains make data warehousing impractical at best, and infeasible at worst.
...
The solution to this is to use a real-time data integration and replecation to a centralized data warehouse. \cite{goldengate} 
...
isolated from the transaction processing sytems, and all of the analytics can be performed in one place on a single dataset. In addition to finding
...
With a data warehouse collecting all of our real time booking reservations, we can slice the data by varios dimensions. For instance, we 
